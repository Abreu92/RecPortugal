        ...
        <div class="max-w-4xl mx-auto px-4">
                            ...
                            <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center gap-4 transition hover:border-zinc-700">
                                ...
                                <div class="w-20 h-20 bg-zinc-800 rounded-lg overflow-hidden flex-shrink-0">
                                    @if(isset($details['image']))
                                        <img src="{{ asset('storage/' . $details['image']) }}" alt="{{ $details['name'] ?? 'Produto' }}" class="w-full h-full object-cover">
                                    @else
                                ...
                                <div class="flex-grow">
                                    <h3 class="font-bold text-lg">{{ $details['name'] ?? 'Produto sem nome' }}</h3>
                                    <p class="text-rec-gold-600 font-mono font-bold">{{ number_format($price, 2) }} €</p>
                                </div>
            ...
            @else
                <div class="text-center py-20 bg-zinc-900 rounded-2xl border border-zinc-800">
                    ...
                    <a href="{{ url('/') }}" class="mt-6 inline-block bg-zinc-800 px-6 py-2 rounded-lg hover:bg-zinc-700 transition">Voltar à Página Inicial</a>
                </div>
            @endif
        </div>
    </div>
